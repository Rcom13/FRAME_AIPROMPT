...
type RigCharacter={
  ...
  detailed?:DetailedBody;
};

type DetailedBody={mesh:THREE.Mesh;basePositions:Float32Array;skinIndices:Uint8Array;skinWeights:Float32Array;restInverse:THREE.Matrix4[];gender:PoseGender};
...
const PoseRig=forwardRef<PoseRigHandle,{ariaLabel:string;realistic:boolean;onPoseChange?:(pose:JointMap)=>void;onPeopleChange?:(people:PoseGender[],active:number)=>void}>(({ariaLabel,realistic,onPoseChange,onPeopleChange},ref)=>{
  ...
  useEffect(()=>{
    ...
    const characters:RigCharacter[]=[];let activeIndex=0,currentView:RigView="front",dragAxis:DragAxis="screen",disposed=false;
    const whiteMaterial=()=>new THREE.MeshStandardMaterial({color:0xf1eee8,roughness:.48,metalness:.03,side:THREE.DoubleSide});
    const handleMaterial=()=>new THREE.MeshStandardMaterial({color:0xff6b35,emissive:0x46170b,roughness:.28,metalness:.25,transparent:true,opacity:.92,depthTest:false});
    ...
    const detailedGeometryCache=new Map<PoseGender,Promise<THREE.BufferGeometry>>();const detailedSkinCache=new Map<PoseGender,{basePositions:Float32Array;skinIndices:Uint8Array;skinWeights:Float32Array;restInverse:THREE.Matrix4[]}>();
    const sourceBodyPose=(gender:PoseGender)=>restPose(gender);
    const segmentMatrix=(a:THREE.Vector3,b:THREE.Vector3)=>{const delta=b.clone().sub(a),length=Math.max(.001,delta.length()),matrix=new THREE.Matrix4(),rotation=new THREE.Quaternion().setFromUnitVectors(up,delta.normalize());return matrix.compose(a.clone().add(b).multiplyScalar(.5),rotation,new THREE.Vector3(1,length,1))};
    const distanceToSegmentSq=(point:THREE.Vector3,a:THREE.Vector3,b:THREE.Vector3)=>{const segment=b.clone().sub(a),lengthSq=segment.lengthSq();if(lengthSq<1e-8)return point.distanceToSquared(a);const amount=THREE.MathUtils.clamp(point.clone().sub(a).dot(segment)/lengthSq,0,1);return point.distanceToSquared(a.clone().addScaledVector(segment,amount))};
    const loadDetailedGeometry=(gender:PoseGender)=>{const cached=detailedGeometryCache.get(gender);if(cached)return cached;const promise=new Promise<THREE.BufferGeometry>((resolve,reject)=>{new OBJLoader().load(`/models/pose-${gender}.obj`,object=>{let source:THREE.BufferGeometry|undefined;object.traverse(child=>{if(!source&&child instanceof THREE.Mesh)source=(child.geometry as THREE.BufferGeometry).clone()});if(!source){reject(new Error("POSE_MODEL_EMPTY"));return}source.deleteAttribute("normal");source.deleteAttribute("uv");const geometry=mergeVertices(source,.0001);source.dispose();geometry.computeBoundingBox();const box=geometry.boundingBox;if(!box){geometry.dispose();reject(new Error("POSE_MODEL_BOUNDS"));return}const height=box.max.y-box.min.y,scale=5.2/Math.max(height,.001),centerX=(box.min.x+box.max.x)/2,centerZ=(box.min.z+box.max.z)/2;geometry.translate(-centerX,-box.min.y,-centerZ);geometry.scale(scale,scale,scale);geometry.translate(0,-2.15,0);geometry.computeVertexNormals();geometry.computeBoundingSphere();resolve(geometry)},undefined,reject)});detailedGeometryCache.set(gender,promise);return promise};
    const deformDetailedBody=(rig:RigCharacter)=>{const body=rig.detailed;if(!body)return;const maps=bonePairs.map(([from,to],index)=>segmentMatrix(rig.joints[from].position,rig.joints[to].position).multiply(body.restInverse[index]));const position=body.mesh.geometry.getAttribute("position") as THREE.BufferAttribute,temp=new THREE.Vector3(),first=new THREE.Vector3(),second=new THREE.Vector3();for(let index=0;index<position.count;index++){const offset=index*3,tempOffset=index*2;temp.fromArray(body.basePositions,offset);first.copy(temp).applyMatrix4(maps[body.skinIndices[tempOffset]]);second.copy(temp).applyMatrix4(maps[body.skinIndices[tempOffset+1]]);first.multiplyScalar(body.skinWeights[tempOffset]).addScaledVector(second,body.skinWeights[tempOffset+1]);position.setXYZ(index,first.x,first.y,first.z)}position.needsUpdate=true};
    const refreshDetailedBody=(rig:RigCharacter)=>{if(!rig.detailed)return;rig.detailed.mesh.geometry.computeVertexNormals();rig.detailed.mesh.geometry.computeBoundingSphere()};
    const fallbackParts=(rig:RigCharacter)=>[...rig.bones,rig.head,rig.torso,rig.pelvisBody,...rig.hands,...rig.feet];
    const removeDetailedBody=(rig:RigCharacter)=>{if(!rig.detailed)return;rig.group.remove(rig.detailed.mesh);rig.detailed.mesh.geometry.dispose();const material=rig.detailed.mesh.material as THREE.Material;material.dispose();rig.detailed=undefined};
    const installDetailedBody=async(rig:RigCharacter,gender:PoseGender)=>{
      try{
        const shared=await loadDetailedGeometry(gender);if(disposed||!characters.includes(rig)||rig.gender!==gender)return;
        let skin=detailedSkinCache.get(gender);
        if(!skin){
          const sourcePose=sourceBodyPose(gender),sourceVectors=Object.fromEntries(JOINT_NAMES.map(name=>[name,new THREE.Vector3(...sourcePose[name])])) as Record<string,THREE.Vector3>;
          const restInverse=bonePairs.map(([from,to])=>segmentMatrix(sourceVectors[from],sourceVectors[to]).invert());
          const positions=shared.getAttribute("position") as THREE.BufferAttribute,basePositions=new Float32Array(positions.array as ArrayLike<number>),skinIndices=new Uint8Array(positions.count*2),skinWeights=new Float32Array(positions.count*2),point=new THREE.Vector3();
          const leftArm=[3,4,5],rightArm=[6,7,8],leftLeg=[9,10,11],rightLeg=[12,13,14],core=[0,1,2,3,6,9,12],pelvisY=sourcePose.pelvis[1],neckY=sourcePose.neck[1],shoulderX=Math.abs(sourcePose.shoulderR[0]);
          for(let vertex=0;vertex<positions.count;vertex++){
            point.fromBufferAttribute(positions,vertex);
            const candidates=point.y>neckY+.08?[0,1]:point.y<pelvisY+.1?(point.x<0?leftLeg:rightLeg):Math.abs(point.x)>shoulderX*.72?(point.x<0?leftArm:rightArm):core;
            const nearest=candidates.map(index=>{const [from,to]=bonePairs[index];return{index,distance:distanceToSegmentSq(point,sourceVectors[from],sourceVectors[to])}}).sort((a,b)=>a.distance-b.distance).slice(0,2);
            const firstWeight=1/(nearest[0].distance+.002),secondWeight=1/(nearest[1].distance+.002),total=firstWeight+secondWeight,offset=vertex*2;
            skinIndices[offset]=nearest[0].index;skinIndices[offset+1]=nearest[1].index;skinWeights[offset]=firstWeight/total;skinWeights[offset+1]=secondWeight/total;
          }
          skin={basePositions,skinIndices,skinWeights,restInverse};detailedSkinCache.set(gender,skin);
        }
        removeDetailedBody(rig);const geometry=shared.clone(),mesh=new THREE.Mesh(geometry,whiteMaterial());mesh.castShadow=true;mesh.receiveShadow=true;mesh.frustumCulled=false;mesh.name=`FRAME-${gender}-mannequin`;rig.detailed={mesh,...skin,gender};rig.group.add(mesh);fallbackParts(rig).forEach(part=>part.visible=false);deformDetailedBody(rig);refreshDetailedBody(rig);
      }catch{if(!disposed&&characters.includes(rig))fallbackParts(rig).forEach(part=>part.visible=true)}
    };
    ...
    const createCharacter=(gender:PoseGender)=>{
      ...
      const rig:RigCharacter={gender,group,joints,bones,head,torso,pelvisBody,hands,feet};characters.push(rig);updateVisuals(rig);layout();setActive(characters.length-1);void installDetailedBody(rig,gender);return rig;
    };
    ...
    const raycaster=new THREE.Raycaster(),pointer=new THREE.Vector2(),dragPlane=new THREE.Plane(),hitPoint=new THREE.Vector3(),dragStartPosition=new THREE.Vector3();let selected:THREE.Mesh|null=null,selectedRig:RigCharacter|null=null,dragStartX=0,dragStartY=0;
    ...
    apiRef.current={capture:()=>{const rig=activeRig();if(rig)refreshDetailedBody(rig);renderer.render(scene,camera);return renderer.domElement.toDataURL("image/png")},reset:()=>{const rig=activeRig();if(rig)applyPose(rig,"neutral")},applyPreset:preset=>{const rig=activeRig();if(rig)applyPose(rig,preset)},applyJointMap,setRealistic:enabled=>{realisticRef.current=enabled;if(enabled){const rig=activeRig();if(rig)conformToTargets(rig,currentTargets(rig))}},mirror,addPerson:gender=>{if(characters.length<3)createCharacter(gender)},removePerson:()=>{if(characters.length<=1)return;disposeGroup(characters[activeIndex].group);characters.splice(activeIndex,1);characters.forEach((rig,index)=>Object.values(rig.joints).forEach(mesh=>mesh.userData.rig=index));activeIndex=Math.min(activeIndex,characters.length-1);layout();setActive(activeIndex)},selectPerson:setActive,setBodyType:gender=>{const rig=activeRig();if(!rig||rig.gender===gender)return;rig.gender=gender;fallbackParts(rig).forEach(part=>part.visible=true);removeDetailedBody(rig);applyPose(rig,"neutral");void installDetailedBody(rig,gender);notifyPeople()},setView:applyView,setDragAxis:axis=>{dragAxis=axis},setFacing:facing=>{const rig=activeRig();if(!rig)return;const rotations:Record<CharacterFacing,number>={front:0,left:Math.PI/2,right:-Math.PI/2,back:Math.PI};rig.group.rotation.y=rotations[facing];reportPose()}};
    return()=>{disposed=true;cancelAnimationFrame(frame);observer.disconnect();intersectionObserver.disconnect();document.removeEventListener("visibilitychange",onVisibility);renderer.domElement.removeEventListener("pointerdown",onPointerDown);renderer.domElement.removeEventListener("pointermove",onPointerMove);renderer.domElement.removeEventListener("pointerup",onPointerUp);renderer.domElement.removeEventListener("pointercancel",onPointerUp);controls.dispose();characters.forEach(rig=>disposeGroup(rig.group));detailedGeometryCache.forEach(promise=>{void promise.then(geometry=>geometry.dispose()).catch(()=>{})});renderer.dispose();renderer.domElement.remove()};
  },[]);
